...
class Router{
    ...
    public function __construct(){
        if(parse_url(ROOT_DIR) === false){
            if(!file_exists(ROOT_DIR . 'app/views/404.php') && $_SERVER['HTTP_HOST'] === 'localhost'){
        ...
        foreach ($this->routes as $key => $value) {
            if(preg_match("@^{$value}$@D", $uriPath, $matches)){
                ...
                if(is_string($this->callback[$key])){
                    ...
                    if(file_exists(ROOT_DIR . 'app/controllers/' . $className . '.Controller.php')){
                        require_once ROOT_DIR . 'app/controllers/' . $className . '.Controller.php';
